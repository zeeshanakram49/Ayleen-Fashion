...
export async function processStripePayment(
  ...
  try {
    ...
    throw new Error(
      "No checkout URL or Stripe session key returned by the backend.",
    );
  } catch (err: unknown) {
    const error = err as Error;
    return {
      success: false,
      message: error.message || "Stripe initialization failed",
    };
  }
}
...
export async function processJazzCashPayment(
  ...
  try {
    ...
    if (!response.success || !response.payload) {
      throw new Error(
        response.message || "Failed to initiate JazzCash transaction",
      );
    }
    ...
    throw new Error(
      "JazzCash response did not contain redirect endpoints or instructions.",
    );
  } catch (err: unknown) {
    const error = err as Error;
    return {
      success: false,
      message: error.message || "JazzCash payment failed",
    };
  }
}
...
export async function processEasyPaisaPayment(
  ...
  try {
    ...
    if (!response.success || !response.payload) {
      throw new Error(
        response.message || "Failed to initiate EasyPaisa transaction",
      );
    }
    ...
    throw new Error(
      "EasyPaisa response did not contain redirect endpoints or instructions.",
    );
  } catch (err: unknown) {
    const error = err as Error;
    return {
      success: false,
      message: error.message || "EasyPaisa payment failed",
    };
  }
}
