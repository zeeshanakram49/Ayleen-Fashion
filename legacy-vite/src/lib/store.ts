...
export const orderTotal = (subtotal: number) =>
  subtotal + shippingFee(subtotal) + taxAmount(subtotal);
...
export function parseHash(): Route {
  ...
  if (base === "product" && maybeSlug)
    return { page: "product", slug: maybeSlug };
  ...
  if (base === "order-success")
    return { page: "orderSuccess", orderId: maybeSlug };
  if (base === "order-failed")
    return { page: "orderFailed", orderId: maybeSlug };
  if (base === "payment-processing")
    return { page: "paymentProcessing", orderId: maybeSlug };
