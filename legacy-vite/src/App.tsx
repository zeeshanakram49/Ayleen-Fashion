...
function readStoredWishlist() {
  try {
    ...
    const storedIds = storedWishlist
      ? (JSON.parse(storedWishlist) as unknown)
      : [];
...
function App() {
  ...
  const [saleEssentialsProducts, setSaleEssentialsProducts] = useState<
    Product[]
  >([]);
  ...
  const [placedPayment, setPlacedPayment] = useState<
  ...
  const loadCatalog = useCallback(async (control?: { cancelled: boolean }) => {
    ...
    try {
      const [nextCatalog, focusData, mustHavesData, saleEssentialsData] =
        await Promise.all([
          fetchCatalog(),
  ...
  useEffect(() => {
    ...
    setCart((prev) =>
      prev.filter((line) => validProductIds.has(line.productId)),
    );
  ...
  function navigateToShop(categoryId = "all", nextQuery = "") {
    ...
    navigateToHash(
      queryString ? `${APP_ROUTES.shop}?${queryString}` : APP_ROUTES.shop,
    );
  }
  ...
  return (
    <div className="premium-scene min-h-screen text-[var(--ink)]">
      ...
      {notice && (
        <div className="fixed top-24 right-4 z-[60] max-w-sm rounded-[var(--radius-md)] border border-[var(--line)] bg-[var(--paper)] px-4 py-3 text-sm shadow-[var(--shadow-lift)]">
