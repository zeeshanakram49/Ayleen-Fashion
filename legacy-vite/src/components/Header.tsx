...
export function Header({
  ...
  const activeDesktopCategory =
    activeDesktopMenu?.categoryId === "all"
      ...
      : categories.find(
          (category) => category.id === activeDesktopMenu?.categoryId,
        );
    ...
    <>
      <header
        className={`site-header transition-all duration-300 ${
          isTransparent
            ? "fixed top-0 right-0 left-0 z-50 border-b border-white/10 bg-gradient-to-b from-black/80 via-black/35 to-transparent text-white"
            : isScrolled
              ? "sticky top-0 z-50 border-b border-black/8 bg-white/95 text-[var(--ink)] shadow-md backdrop-blur-xl"
              : "sticky top-0 z-50 border-b border-black/8 bg-white text-[var(--ink)]"
        }`}
      >
        {/* Top Announcement Bar */}
        <div
          className={`site-announcement-bar px-4 py-2 text-center text-[11px] font-medium tracking-[0.2em] uppercase transition-colors ${
            isTransparent
              ? "border-b border-white/10 bg-black/40 text-neutral-200"
        ...
        >
          ...
          <a
            ...
            className={`site-wordmark font-display shrink-0 text-3xl font-extrabold tracking-tighter transition ${
          ...
          <nav
            className="flex items-center gap-8 text-sm font-semibold tracking-[0.14em] uppercase"
            ...
            {desktopLinks.map((link) => (
              <button
                ...
                className={`nav-link-underline py-2 transition-colors ${
                  isTransparent
                    ? "text-white drop-shadow-sm hover:text-neutral-200"
                    : (
                          link.id === "men"
                            ? menMenuIsActive
            ...
            <button
              ...
              className={`p-2 transition active:scale-95 ${
                isTransparent
                  ...
                  : "text-neutral-800 hover:text-[var(--ink)]"
              }`}
          ...
          <AnimatePresence>
            {activeDesktopMenu && (
              <motion.div
                ...
                className="site-mega-menu absolute top-full left-0 w-full border-t border-[var(--line)] bg-white/98 p-8 shadow-2xl backdrop-blur-2xl"
                onMouseEnter={clearDesktopMenuClose}
                onMouseLeave={closeDesktopMenuSoon}
                onWheel={keepMegaMenuScroll}
              >
                <div className="mx-auto grid max-w-6xl grid-cols-3 gap-12">
                  <div className="col-span-1 space-y-4">
                    <h3 className="border-b border-[var(--line)] pb-2 text-xs font-bold tracking-[0.2em] text-[var(--muted)] uppercase">
                    ...
                    <div className="flex flex-col gap-2">
                      {activeDesktopMenu.items.map((item) => (
                        <button
                          ...
                          className="flex items-center justify-between py-1 text-left text-sm font-semibold tracking-wider text-[var(--ink)] uppercase transition duration-200 hover:translate-x-1 hover:text-[var(--muted)]"
                        >
                  ...
                  <div className="relative col-span-2 h-64 overflow-hidden rounded-2xl bg-[var(--panel)]">
                    <ImageWithFallback
                    ...
                    <div className="absolute inset-0 flex flex-col justify-end bg-gradient-to-t from-black/70 via-black/20 to-transparent p-6 text-white">
                      ...
                      <button
                        ...
                        className="mt-3 inline-flex items-center gap-2 rounded-full bg-white px-5 py-2 text-xs font-bold tracking-widest text-[var(--ink)] uppercase transition hover:bg-neutral-200"
                      >
        ...
        <AnimatePresence>
          {menuOpen && (
            <motion.div
              ...
              className="fixed inset-0 top-[100px] z-[9970] flex flex-col overflow-y-auto bg-white p-6 shadow-2xl lg:hidden"
            >
              <div className="flex flex-col gap-3">
                ...
                <button
                  ...
                  className="flex items-center justify-between rounded-xl bg-[var(--panel)] p-4 text-sm font-bold tracking-wider text-[var(--ink)] uppercase"
                >
                ...
                {menCategories.map((category) => (
                  <button
                    ...
                    className="flex items-center justify-between rounded-xl border border-[var(--line)] p-4 text-sm font-bold tracking-wider text-[var(--ink)] uppercase transition hover:bg-[var(--panel)]"
                  >
