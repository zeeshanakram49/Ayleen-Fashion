...
export function normalizeOrder(raw: unknown): Order | null {
  ...
  const postCode = firstString(
    [addressRecord],
    ["post_code", "postCode", "zip"],
  );

  const fullAddress = [address1, address2, city, postCode, country]
    .filter(Boolean)
    .join(", ");
  ...
  const items = rawItems.map((item: unknown) => {
    ...
    return {
      ...
      productTitle:
        firstString(
          [itemObj],
          ["product_name", "productTitle", "title", "name"],
        ) || "Product",
  ...
  const total = firstNumber(
    [orderRecord],
    ["total_amount", "total", "grand_total", "payable_amount"],
  );
  const subtotal =
    firstNumber([orderRecord], ["sub_total", "subtotal"]) || total;
  const paymentStatusRaw =
    firstString([orderRecord], ["payment_status", "paymentStatus"]) || "unpaid";

  return {
    id:
      firstString([orderRecord], ["id", "order_id", "orderId"]) || orderNumber,
    orderNumber,
    customerName,
    customerEmail: firstString(
      [addressRecord, orderRecord],
    ...
    items,
    subtotal,
    shippingFee: firstNumber(
      [orderRecord],
      ["shipping_fee", "shipping", "delivery_fee"],
    ),
    ...
    createdAt:
      firstString([orderRecord], ["created_at", "createdAt"]) ||
...
export async function createOrderApi(
  ...
  const orderId =
    ...
    firstString(
      [root],
      ["order_id", "orderId", "order_number", "orderNumber", "id"],
    );
  const total =
    normalized?.total ||
    firstNumber([root], ["total", "grand_total", "total_amount"]);

  if (normalized && typeof window !== "undefined" && orderId) {
    try {
      sessionStorage.setItem(
        `ayleen_order_${orderId}`,
...
export async function fetchOrderDetailApi(
  orderId: string,
