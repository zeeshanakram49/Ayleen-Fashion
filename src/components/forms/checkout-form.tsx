...
export function CheckoutForm() {
   ...
   * META PIXEL
   *
   * Fires InitiateCheckout once when
   * checkout opens with items in the cart.
   */
  useEffect(() => {
    if (!lines.length || checkoutTracked.current) {
      return;
    }
    ...
    const trackInitiateCheckout = () => {
      ...
      const contentIds = lines.map((line) => String(line.productId));

      window.fbq("track", "InitiateCheckout", {
        content_ids: contentIds,
        content_type: "product",
        contents,
        value: Number(summary.subtotal),
        currency: "PKR",
    ...
    const interval = window.setInterval(() => {
      ...
      if (trackInitiateCheckout() || attempts >= 20) {
  ...
  }, [lines, summary.subtotal, summary.itemCount]);
  ...
  async function submit(values: CheckoutFormInput) {
    ...
    try {
       ...
       *
       * The Purchase Meta event will fire only
       * after the customer reaches the successful
      ...
      const purchasePayload = {
        ...
        content_ids: lines.map((line) => String(line.productId)),
       ...
       *
       * For CARD / JazzCash / EasyPaisa the user
       * still needs to complete external payment.
       * If payment fails or is cancelled, their
       * cart should remain available.
       */
