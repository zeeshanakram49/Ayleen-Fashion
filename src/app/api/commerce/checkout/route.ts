...
function findOrderId(value: unknown): string {
  ...
  const nestedOrder = isRecord(record.order)
    ? record.order
    : null;
  ...
  return (
    ...
    asString(record.id) ||
    (nestedOrder
      ? asString(nestedOrder.order_number) ||
...
function findTotal(value: unknown): number {
  ...
  return (
    ...
    (nestedOrder
      ? (asNumber(nestedOrder.total_amount) ??
        asNumber(nestedOrder.total))
      : null) ??
    0
  );
}
...
export async function POST(request: Request) {
  try {
    ...
    const total = findTotal(order.data);
