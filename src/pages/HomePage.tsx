  ...
  return (
    <>
      <section className="hero-cinematic relative overflow-hidden border-b border-[var(--line)]">
        ...
        <div className="mx-auto grid max-w-7xl gap-10 px-6 py-14 md:py-20 lg:grid-cols-[1.15fr_1fr] lg:items-center">
          <div className="reveal-up">
            <p className="text-xs font-semibold tracking-[0.35em] text-[var(--gold-deep)]">SPRING SUMMER 2026</p>
            <h1 className="font-editorial mt-5 max-w-xl text-5xl leading-[0.92] sm:text-6xl lg:text-7xl">
              Elevated everyday fashion by AYLEEN.
            </h1>
            <p className="mt-6 max-w-xl text-sm leading-7 text-[var(--muted)] sm:text-base">
              A premium Pakistani fashion destination blending timeless cuts, confident tailoring, and modern street energy for women and men.
            </p>

            <div className="mt-8 flex flex-wrap gap-4">
              <a
                href="#/shop"
                className="magnetic-btn rounded-full bg-[var(--ink)] px-7 py-3 text-xs font-semibold tracking-[0.2em] text-[var(--champagne)] transition hover:-translate-y-0.5 hover:shadow-lg"
              >
                SHOP NOW
              </a>
              <a
                href="#/about"
                className="ghost-btn rounded-full border border-[var(--ink)]/70 px-7 py-3 text-xs font-semibold tracking-[0.2em] transition hover:border-[var(--gold-deep)] hover:text-[var(--gold-deep)]"
              >
                BRAND STORY
              </a>
            </div>
      ...
      <section className="mx-auto max-w-7xl px-6 py-16 md:py-24" id="categories">
        ...
        <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-4">
          {categories.map((category, index) => (
            <article
              key={category.id}
              className="group lift-strong shimmer-border reveal-up soft-shadow relative overflow-hidden rounded-[1.8rem]"
